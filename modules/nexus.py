...
class Nexus(CommonMethods):
    """
    Class for a Cisco NX-OS based system
    """
    def __init__(self, crt=None, ticket=None):
        ...
        self.bios_version_regex = re.compile(r'BIOS: *version *\d+\.\d+\.\d+', re.IGNORECASE)
        self.image_version_regex = re.compile(r'system: *version *\d+\.\d+\(\d+\)', re.IGNORECASE)
        self.kickstart_version_regex = re.compile(r'kickstart: *version *\d+\.\d+\(\d+\)', re.IGNORECASE)
        # self.config_register_regex = re.compile(r'\d+X\d+', re.IGNORECASE)
        self.bgp_regex = re.compile(r'feature bgp')
        self.eigrp_regex = re.compile(r'feature eigrp')
        self.ospf_regex = re.compile(r'feature ospf')
        self.pim_regex = re.compile(r'feature pim')
    ...
    def get_image_version(self):
        ...
        found_data = self._get_regex_data_from_pull('show version\r\n', '{hostname}#'.format(hostname=self.hostname),
                                                    self.bios_version_regex,
                                                    self.image_version_regex,
                                                    self.kickstart_version_regex,
                                                    wait_time=5)

        if len(found_data) >= 1:

            if found_data[0]:
                self.bios_version = found_data[0]
                LOGGER.debug('Found bios_version {} get_image_version {}'.format(self.bios_version, type(self)))

            else:
                self.bios_version = 'UNKNOWN'

            if found_data[1]:
                self.image_version = found_data[1]
            ...
            if found_data[2]:
                self.kickstart_version = found_data[2]
                LOGGER.debug('Found kickstart_version {} get_image_version {}'.format(self.kickstart_version,
                                                                                      type(self)))

            else:
                self.kickstart_version = 'UNKNOWN'
